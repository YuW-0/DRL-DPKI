...
def train_dqn(n_na, n_episodes, steps_per_episode, lr, 
    ...
    for ep in range(n_episodes):
        ...
        # Update learning rate at the start of each episode (staged decay)
        current_lr = agent.update_learning_rate(ep)
        
        # Record learning-rate and epsilon history
        if ep % 10 == 0:  # Record once every 10 episodes
            agent.lr_history.append(current_lr)
            agent.epsilon_history.append(agent.epsilon)
        
        # Print learning-rate change once every 100 episodes
        if ep % 100 == 0 and ep > 0:
            print(f"Episode {ep}: Learning Rate = {current_lr:.6f}")
        
        state = env.reset()  # Reset environment state, while keeping fixed NA features unchanged
        
        # For the first and last episode, print NA parameters for verification
        if ep == 0:
            print(f"\nFirst episode (#{ep}) fixed NA parameter check:")
            print("id\treputation\tsuccess_rate\tsign_delay(ms)\tdelay_grade\tgroup")
            for i in range(min(n_na, 10)):  # Print first 10 only to avoid long output
                group = "low-reputation" if env.reputation[i] < 6600 else "high-reputation"
        ...
        # Initialize episode total reward and stats
        ep_reward = 0
        ep_step_rewards = []  # Per-step rewards in this episode
        ep_successes = 0  # Success count in this episode
        
        # Record episode data (first and last episode)
        ...
        if is_first_episode or is_last_episode:
            ...
            episode_selected_nas = []  # Selected NA at each step
        
        # If this is the last episode, record initial reputation
        if is_last_episode:
            reputation_last.append(env.reputation.copy())
        
        for t in range(steps_per_episode):
            # Select a single NA; record policy-analysis data every 10 episodes (or on last)
            record_policy = (ep % 10 == 0) or is_last_episode  # Every 10 episodes or last episode
            ...
            # Key change: conditional updates instead of training every step
            batch_losses = agent.maybe_update(global_step=total_training_steps)
            if batch_losses is not None:
                total_updates += len(batch_losses)
                # Print detailed training info only in the last episode
                if ep == n_episodes - 1:
                    avg_loss = np.mean(batch_losses)
                    # Removed detailed loss printing to reduce output
                
            # Accumulate episode reward
            ep_reward += reward
            ep_step_rewards.append(reward)

            # Aggregate rewards across multiple steps to reduce noise
            ...
            # Track successes (by reward sign)
            if reward > 0:
                ep_successes += 1
            
            state = next_state
            
            # Record detailed step-level data for first and last episode
            if is_first_episode or is_last_episode:
                episode_reputation_history.append(env.reputation.copy())
                episode_selected_nas.append(action_na)
                  # Record last-episode reputation after each step
        if is_last_episode:
            reputation_last.append(env.reputation.copy())
        ...
        # Compute Q-value statistics every 10 episodes to reduce overhead
        if ep % 10 == 0:  # Only compute Q stats every 10 episodes
            with torch.no_grad():
        ...
        # Save first and last episode data
        if is_first_episode:
            first_episode_data = {
            ...
            try:
                ...
                print(f"[OK] First-episode NA parameter export succeeded. File: {export_file}")
            except Exception as e:
                print(f"[ERROR] First-episode NA parameter export failed: {e}")
        ...
        if (ep+1) % 20 == 0:
            ...
            print(f"Episode {ep+1}, Reward: {ep_reward:.2f}, Mean Reputation: {env.reputation.mean():.2f}, "
                  ...
                  f"Epsilon: {agent.epsilon:.4f}, {memory_status}, Update Ratio: {update_ratio:.1f}%")
        ...
        if (ep + 1) % 50 == 0 and ep > 0:
            ...
            # Check whether the network learned a reasonable NA-selection strategy
            ...
            if len(low_rep_indices) > 0 and len(high_rep_indices) > 0:
                # Compute the group composition among top-5 Q-value NAs
                ...
                print(f"  Top-5 Q-value NAs: low-reputation {top_5_low_count}, high-reputation {top_5_high_count}")
                
                # Evaluate learning quality
                if top_5_high_count >= 3:
                    print("  [OK] Good learning quality: prioritizes high-reputation NAs")
                elif top_5_high_count >= 2:
                    print("  [WARN] Moderate learning quality: selection strategy may need improvement")
                else:
                    print("  [ERROR] Poor learning quality: hyperparameters may need adjustment")
            
            # Print sliding-window stats every 100 episodes
            if (ep + 1) % 100 == 0:
                env.print_sliding_window_summary(max_nas_to_show=10)
        
        # Print details at key epsilon transition points
        first_transition = int(0.15 * n_episodes)  # First transition: 15%
        second_transition = int(0.5 * n_episodes)  # Second transition: 50%
        
        if ep == first_transition or ep == first_transition + 1:
            print(
                f"Episode {ep+1}: Epsilon first transition, ε = {agent.epsilon:.4f} "
                f"({'very-fast decay -> fast decay' if ep == first_transition else 'fast-decay stage'})"
            )
        elif ep == second_transition or ep == second_transition + 1:
            print(
                f"Episode {ep+1}: Epsilon second transition, ε = {agent.epsilon:.4f} "
                f"({'fast decay -> fine-tuning' if ep == second_transition else 'fine-tuning stage'})"
            )
        elif ep in [n_episodes-10, n_episodes-5, n_episodes-1]:
            print(f"Episode {ep+1}: near training end, ε = {agent.epsilon:.4f}")
    
    # Flush remaining rewards that do not fill a full aggregation window
    if step_reward_buffer:
        averaged_reward = sum(step_reward_buffer) / len(step_reward_buffer)
        step_rewards.append(averaged_reward)
        step_reward_steps.append(total_training_steps)

    # Ensure the last episode has recorded learning rate and epsilon (if not a multiple of 10)
    final_episode = n_episodes - 1  # Last episode index
    if final_episode % 10 != 0:  # If last episode index is not a multiple of 10
        current_lr = agent.update_learning_rate(final_episode)
        agent.lr_history.append(current_lr)
        agent.epsilon_history.append(agent.epsilon)
        print(f"Recorded last episode {final_episode}: Learning Rate = {current_lr:.6f}, Epsilon = {agent.epsilon:.6f}")
    
    # Print training statistics
    print("\nTraining statistics:")
    print(f"Total training steps: {total_training_steps}")
    print(f"Total network updates: {total_updates}")
    print(f"Update frequency: try training once every {update_frequency} steps")
    print(f"Actual update ratio: {total_updates/max(1, total_training_steps)*100:.2f}%")
    print(f"Target network updates: {agent.learn_step//agent.target_update}")
    print(f"Final epsilon: {agent.epsilon:.4f}")
    
    # Key post-training sanity checks
    print("\nKey sanity checks after training:")
    
    # 1) Check whether network parameters were updated
    final_state = env.get_state()
    with torch.no_grad():
        initial_q = torch.zeros(n_na)  # Hypothetical initial Q values
    ...
    if q_magnitude < 0.1:
        print("  [ERROR] Network may be undertrained")
    ...
    print(f"  Replay buffer utilization: {len(agent.memory)}/{agent.min_memory_size} "
          f"({len(agent.memory)/agent.min_memory_size*100:.1f}%)")
    
    # 3) Check whether training frequency is reasonable
    expected_updates = total_training_steps // update_frequency
    update_efficiency = total_updates / max(1, expected_updates) * 100
    print(f"  Training efficiency: {update_efficiency:.1f}% (expected {expected_updates}, actual {total_updates})")
    ...
    if len(agent.loss_history) > 100:
        ...
        if loss_improvement > 20:
            ...
            print("  [ERROR] No clear loss improvement; check hyperparameters")
    else:
        print("  [WARN] Not enough training data to evaluate loss convergence")
    
    # Make the final selection using the trained DQN
    print("\n" + "="*80)
    print("Final NA selection using the trained DQN")
    print("Training mode: fixed NA set; choose one NA per step; final set formed by repeated selections")
    print("="*80)
    ...
    print("id\tinit_rep\tcurr_rep\trep_change\tsuccess_rate\tsign_delay(ms)\tdelay_grade\tQ\t\tgroup")
    for i in range(min(n_na, 15)):  # Limit output length
    ...
    print(f"\n[DQN] Final NA set of size {len(dqn_selected)} formed via repeated single selections (target: 2 low + 3 high):")
    print("id\tinit_rep\tcurr_rep\trep_change\tsuccess_rate\tsign_delay(ms)\tdelay_grade\tQ\t\tgroup\t\tmalicious")
    dqn_low_count = 0
    dqn_high_count = 0
    ...
    for i in dqn_selected:
        ...
        # Check whether this is a malicious NA and get its type
        ...
        if is_malicious:
            ...
            if mal_type == 1:
                ...
                malicious_info = "malicious T4 (traditional low success)"
            else:
    ...
    # Show malicious-NA statistics for DQN selection
    if dqn_malicious_count > 0:
        print(
            f"[WARN] DQN selection contains {dqn_malicious_count} malicious NAs. "
            f"This suggests successful evasion or insufficient detection."
        )
    
    print(f"\n[RANDOM] Randomly selected NA set of size {len(random_selected)} (baseline):")
    print("id\tinit_rep\tcurr_rep\trep_change\tsuccess_rate\tsign_delay(ms)\tdelay_grade\tQ\t\tgroup\t\tmalicious")
    random_low_count = 0
    random_high_count = 0
    random_total_reputation = 0
    random_total_q_value = 0
    random_malicious_count = 0  # Count malicious NAs selected randomly
    ...
    # Show malicious-NA statistics for random selection
    if random_malicious_count > 0:
        print(f"[INFO] Random selection includes {random_malicious_count} malicious NAs (expected for random choice).")
    
    # Simulate training process for the selected NAs
    dqn_training_results = simulate_selected_nas_training(env, dqn_selected, "DQN selected NA training simulation", steps=100)
    
    # Show sliding-window statistics summary
    env.print_sliding_window_summary(max_nas_to_show=15)
    random_training_results = simulate_selected_nas_training(env, random_selected, "Randomly selected NA training simulation", steps=100)
    
    print("\nSelection strategy comparison (full training simulation):")
    print(f"{'Metric':<25} {'DQN':<15} {'Random':<15} {'DQN Advantage':<15}")
    print("-" * 70)
    print(f"{'Low-rep count':<25} {dqn_low_count:<15} {random_low_count:<15} {'OK' if dqn_low_count >= 2 else 'NO':<15}")
    print(f"{'High-rep count':<25} {dqn_high_count:<15} {random_high_count:<15} {'OK' if dqn_high_count >= 3 else 'NO':<15}")
    print(
        f"{'Malicious NA count':<25} {dqn_malicious_count:<15} {random_malicious_count:<15} "
        f"{('less' if dqn_malicious_count < random_malicious_count else ('equal' if dqn_malicious_count == random_malicious_count else 'more')):<15}"
    )
    
    # Compute post-training summary stats
    ...
    print(f"{'Avg rep (before)':<25} {dqn_avg_initial_rep:<15.2f} {random_avg_initial_rep:<15.2f} {dqn_avg_initial_rep - random_avg_initial_rep:+.2f}")
    print(f"{'Avg rep (after)':<25} {dqn_avg_final_rep:<15.2f} {random_avg_final_rep:<15.2f} {dqn_avg_final_rep - random_avg_final_rep:+.2f}")
    print(f"{'Avg rep change':<25} {dqn_avg_rep_change:<15.2f} {random_avg_rep_change:<15.2f} {dqn_avg_rep_change - random_avg_rep_change:+.2f}")
    print(
        f"{'Transaction success rate':<25} {dqn_avg_transaction_success_rate:<15.1%} {random_avg_transaction_success_rate:<15.1%} "
        f"{'+' if dqn_avg_transaction_success_rate > random_avg_transaction_success_rate else ''}{(dqn_avg_transaction_success_rate - random_avg_transaction_success_rate):.1%}"
    )
    ...
    # Create selection-strategy comparison animation
    print("\nCreating selection strategy comparison animation...")
    ...
    if len(low_rep_indices) > 0:
        ...
        print(f"Low-rep Q-value range: {low_q_values.min():.4f} ~ {low_q_values.max():.4f} (avg: {low_q_values.mean():.4f})")
    if len(high_rep_indices) > 0:
        high_q_values = all_q_values[high_rep_indices]
        print(f"High-rep Q-value range: {high_q_values.min():.4f} ~ {high_q_values.max():.4f} (avg: {high_q_values.mean():.4f})")
    ...
    print(f"Signature delay range: {min(all_delays):.1f}ms ~ {max(all_delays):.1f}ms (avg: {np.mean(all_delays):.1f}ms)")
    print(f"Delay grade range: {min(all_delay_perfs):.3f} ~ {max(all_delay_perfs):.3f} (avg: {np.mean(all_delay_perfs):.3f})")
    ...
    if len(low_rep_indices) > 0:
        ...
        print(f"Low-rep delay: {np.mean(low_delays):.1f}ms (grade: {np.mean(low_delay_perfs):.3f})")
    if len(high_rep_indices) > 0:
        high_delays = [all_delays[i] for i in high_rep_indices]
        high_delay_perfs = [all_delay_perfs[i] for i in high_rep_indices]
        print(f"High-rep delay: {np.mean(high_delays):.1f}ms (grade: {np.mean(high_delay_perfs):.3f})")
    
    # Reminder about delay-grade definition
    print("Delay grade: range 0.0-1.0 (0.0 = fastest, 1.0 = slowest)")
    
    # Correlation analysis between Q values and NA composite quality
    print("\nQ-value learning effectiveness analysis:")
    print("id\tinit_rep\tsuccess\tQ\t\tsign_delay_grade\thunger\tdelay_grade\tcomposite\tjudgement")
    
    # Compute composite quality score (0-1 range) for each NA
    na_scores = []
    for i in range(n_na):
        initial_rep = env.episode_start_reputation[i]
        success_rate = env.success_rate[i]
        delay_perf = env.calculate_delay_performance(env.signature_delay[i])
        # Composite: success 40% + delay 30% + reputation 20% + hunger 10%
    ...
    for i in range(n_na):
        ...
        # Q values should be positively correlated with composite score
        # High-score NAs should have high Q values, and vice versa
        ...
        # Check whether Q-rank and score-rank are consistent (allow ±0.3 tolerance)
    ...
    print(f"Correlation coefficient (Q vs composite score): {correlation:.3f}")
    if correlation > 0.7:
        print("[OK] Excellent: Q values reflect NA composite quality well")
    elif correlation > 0.5:
        print("[OK] Good: Q values generally reflect NA composite quality")
    elif correlation > 0.3:
        print("[WARN] Moderate: Q values partially reflect NA composite quality; may need more training")
    else:
        print("[ERROR] Poor: weak correlation between Q values and NA composite quality; check training settings")
    ...
    for i in range(n_na):
        # Different style depending on whether NA was selected
        if i in selected_idx:
            # Selected by DQN: thick line with markers
    ...
    # Optional: generate NA reputation GIF animations for the first and last episodes
    ...
    # Save step-level reward history for plotting
...
def create_episode_reputation_animation(episode_data, n_na, title_prefix):
    """
    Create a GIF animation of NA reputation dynamics for a single episode.
    ...
    lines = []
    points = []
    selected_markers = []  # Markers for selected NAs
    ...
    def animate(frame):
        ...
        # Update each NA's line and point
        for i in range(n_na):
            # Get all data points up to the current step
    ...
    # Create animation (initial state + each step), faster playback
    total_frames = len(reputation_history)
    anim = animation.FuncAnimation(fig, animate, frames=total_frames,
                                 interval=400, blit=True, repeat=True)  # 800ms -> 400ms
    ...
    try:
        ...
        anim.save(full_path, writer='pillow', fps=2.5)  # 1.25 -> 2.5 FPS
        print(f"[OK] {title_prefix} GIF animation saved as: {full_path}")
    except Exception as e:
        print(f"[ERROR] Error saving {title_prefix} animation: {e}")
...
def create_comparison_strategy_gif(env, dqn_selected, random_selected, dqn_training_results, random_training_results, steps=100):
    """
    Create an animation comparing selection strategies, showing reputation trajectories
    for DQN-selected vs randomly selected NAs during the simulated training.
    
    Args:
        env: Environment object.
        dqn_selected: List of DQN-selected NA indices.
        random_selected: List of randomly selected NA indices.
    ...
    # Collect DQN-selected NA reputation histories
    ...
    if not all_reputations:
        print("[ERROR] Not enough data to create animation")
        return
    ...
    try:
        ...
        print(f"[OK] Strategy comparison animation saved: {full_path}")
        ...
        print(f"[ERROR] Failed to save strategy comparison animation: {e}")
...
if __name__ == "__main__":
    ...
    print(f"\n[OK] Training finished. Generated {len(df)} episodes of training data.")
    print(f"Loss history entries: {len(loss_history)} training steps")
    ...
    try:
        ...
        print(f"   - Policy net state dict: {policy_state_path}")
        print(f"   - Target net state dict: {target_state_path}")
    ...
    if hasattr(agent, 'lr_history') and agent.lr_history:
        ...
        if hasattr(agent, 'epsilon_history') and agent.epsilon_history:
            line2 = ax2.plot(lr_episodes, agent.epsilon_history, label='Epsilon (exploration)', color='purple', linewidth=2)
    ...
    print(f"Training results plot saved: {main_png}")
    ...
    if len(loss_history) > 0:
        ...
        print("\n[WARN] No loss data recorded; training may be too short or batch_size too large")
    ...
    print(f"Early: mean reward={early_episodes['reward'].mean():.2f}, std={early_episodes['reward'].std():.2f}")
    print(f"Mid: mean reward={middle_episodes['reward'].mean():.2f}, std={middle_episodes['reward'].std():.2f}")
    print(f"Late: mean reward={late_episodes['reward'].mean():.2f}, std={late_episodes['reward'].std():.2f}")
    ...
    print(f"   - Total episodes: {len(df)}")
    print(f"   - Avg time/episode: {total_duration/len(df):.2f}s")
    if len(loss_history) > 0:
        print(f"   - Total training steps: {len(loss_history)}")
        print(f"   - Avg time/step: {total_duration/len(loss_history)*1000:.2f}ms")
    print(f"="*80)
    print("Done.")
    ...
    try:
        ...
        print(f"[OK] NA selection parameters exported: {export_file}")
        
    except Exception as e:
        print(f"[ERROR] Failed to export NA selection parameters: {e}")
    
    print(f"\n" + "="*80)
    print("Network flexibility demo - varying NA counts")
    print("="*80)
    
    def test_network_flexibility(trained_agent):
        """Demonstrate handling different NA counts with a trained network."""
        ...
        for n_test_na in test_cases:
            ...
            print(f"[OK] NA count: {n_test_na:2d} -> Q shape: {q_values.shape} | max Q: {q_values.max().item():.4f}")
        
        print("[OK] The network handled all tested NA counts.")
        print("Note: the trained model can be applied to any NA count with the same feature format.")
